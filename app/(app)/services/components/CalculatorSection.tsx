...
export default function CalculatorSection() {
    ...
    const selectStyle: React.CSSProperties = {
        ...
        background: CARD, color: B, fontSize: 15, width: "100%",
        outline: "none", cursor: "pointer", appearance: "none",
    };

    function Sel({ label, value, set, items }: { label: string; value: number; set: (v: number) => void; items: [string, number][] }) {
        return (
            <label style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 14, fontWeight: 600, color: B }}>
                {label}
                <select value={value} onChange={e => set(Number(e.target.value))} style={selectStyle}>
                    {items.map(([l, v]) => <option key={v} value={v}>{l}</option>)}
                </select>
            </label>
        );
    }
                ...
                <div className="grid lg:grid-cols-[1fr_360px] gap-5">
                    <Card style={{ padding: 24 }}>
                        <div className="grid md:grid-cols-2 gap-5">
                            <Sel label="Материал" value={mat} set={setMat} items={[["ПВХ 3 мм", 1], ["Акрил", 1.35], ["Металл", 1.6]]} />
                            <Sel label="Формат" value={size} set={setSize} items={[["до A3", 1], ["до A2", 1.45], ["Индивидуальный", 2.1]]} />
                            <label style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 14, fontWeight: 600, color: B }} className="md:col-span-2">
                                ...
                                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: MUTED }}>
                                    <span>1 шт.</span><span>500 шт.</span>
                                </div>
                            </label>
                            <Sel label="Срочность" value={rush} set={setRush} items={[["Стандарт", 1], ["Срочно +35%", 1.35]]} />
                            <Sel label="Доставка" value={delivery} set={setDelivery} items={[["Самовывоз", 0], ["Курьер Москва", 900], ["ТК по России", 1600]]} />
                        </div>
                    </Card>
                    <Card style={{ padding: 24, display: "flex", flexDirection: "column" }}>
                        <p style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.12em", color: MUTED }}>Примерная стоимость</p>
                        <motion.div
                            key={total}
                            initial={{ opacity: 0, y: 8 }}
                            animate={{ opacity: 1, y: 0 }}
                            style={{ marginTop: 16, fontSize: "clamp(36px,5vw,52px)", fontWeight: 900, color: B, lineHeight: 1 }}
                        >
                        ...
                        <a
                            ...
                            style={{
                                ...
                                background: Y, color: B, fontWeight: 700, fontSize: 15, borderRadius: 14, padding: "14px 20px",
