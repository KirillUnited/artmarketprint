...
export default async function ProductsCategoryPage({params, searchParams}: {params: Promise<Props>; searchParams: Promise<{page?: string; sub?: string}>}): Promise<JSX.Element> {
	...
	const [total, categories] = await Promise.all([sanityFetch({query: getTotalProductsQuery(categorySlug, activeSubcategory || null)}), sanityFetch({query: getCategoriesQuery})]);
	...
	return (
		<Section className="space-y-6">
			...
			<h1 className="text-3xl font-bold">
				{activeSubcategory?.title || categorySlug?.title || 'Каталог'} <span
				className="text-sm font-light text-gray-600 truncate">{`${total} шт.`}</span>
			</h1>
			<div className="grid md:grid-cols-[270px,1fr] gap-4">
				<SubCategoryFilter category={categorySlug} categorySlug={category} activeSubcategory={activeSubcategory}
								   baseUrl={BASE_URL}/>
				<div className="space-y-4">
					<Suspense
						fallback={<Loader size="lg" variant="spinner" label="Загрузка товаров..." className="static"/>}>
						<ProductList categorySlug={categorySlug} subcategorySlug={activeSubcategory}
									 pageNumber={pageNumber} PRODUCTS_PER_PAGE={PRODUCTS_PER_PAGE}/>
					</Suspense>
				</div>
			</div>
			{totalPages > 1 && <ClientPagination totalPages={totalPages} pageNumber={pageNumber}
												 basePath={`${BASE_URL}/${category}`}/>}
		</Section>
	);
}
