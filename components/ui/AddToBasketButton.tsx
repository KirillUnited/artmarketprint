...
const AddToBasketButton: React.FC<AddToBasketButtonProps> = ({product}) => {
	...
	const handleAddItem = () => {
		...
		if (itemCount === 0) {
			...
			const category2 = product.subcategory || '';
					...
					items: [
						{
							...
							item_brand: product.brand || 'Без бренда',
...
// GTM push: remove_from_cart
	...
	return (
		<div className="flex flex-wrap gap-4 w-full">
			{itemCount > 0 ? (
				<>
					<QuantityControls itemCount={itemCount} onDecrease={handleRemoveItem} onIncrease={handleAddItem} />
					<ViewBasketButton />
				</>
			) : (
